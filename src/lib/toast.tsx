  ...
  return (
    <Ctx.Provider value={{ notify }}>
      ...
      <div className="fixed bottom-5 right-5 z-[100] flex flex-col gap-2 w-80 max-w-[calc(100vw-2.5rem)]">
        {toasts.map((t) => (
          <div key={t.id} className="card flex items-start gap-3 p-3.5 animate-slide-up" role="status">
            ...
            <button onClick={() => setToasts((arr) => arr.filter((x) => x.id !== t.id))} className="text-fg-muted hover:text-fg">
              <X className="w-4 h-4" />
            </button>
          </div>
        ))}
