...
export default function CoverCreator() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      <PageHeader
        title="Cover Creator"
        subtitle="Design KDP-compliant covers with front, spine, and back"
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="card p-5 space-y-4">
            ...
            <div>
              <label className="label">Page Count (for spine width)</label>
              <input type="number" min={24} className="input" value={pageCount} onChange={(e) => setPageCount(Number(e.target.value))} />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {COLORS.map((c) => (
                  <button key={c} onClick={() => setPrimaryColor(c)} className={`w-8 h-8 rounded-lg border-2 transition ${primaryColor === c ? 'border-fg scale-110' : 'border-transparent'}`} style={{ backgroundColor: c }} />
                ))}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="card p-6">
            <div className="flex items-center gap-2 mb-4">
              <BookImage className="w-5 h-5 text-brand-600" />
              <h3 className="font-semibold text-fg">Cover Preview</h3>
            </div>
            ...
            <div className="space-y-2">
              {issues.map((issue, i) => (
                <div key={i} className="flex items-start gap-2 text-sm">
                  ...
                  <div>
                    <span className="font-medium text-fg-soft">{issue.field}:</span>{' '}
      ...
      <div className="bg-white border-r border-border-soft flex items-center justify-center" style={{ width: fw, height: fh }}>
        <div className="text-center px-4">
          ...
          <div className="mt-auto pt-4">
            <div className="w-16 h-10 border border-slate-300 rounded mx-auto" />
            <div className="text-[7px] text-slate-400 mt-1">Barcode</div>
          </div>
        </div>
      </div>
      {/* Spine */}
      <div className="flex items-center justify-center" style={{ width: sw, height: fh, backgroundColor: primaryColor }}>
        <div className="text-white text-[9px] font-bold whitespace-nowrap" style={{ transform: 'rotate(-90deg)' }}>
          {title} — {author}
        </div>
      </div>
      {/* Front */}
