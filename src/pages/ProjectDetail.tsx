...
import { CircleCheck as CheckCircle2, Circle, Clock, ChevronRight, ArrowLeft, Download, FileText, ShieldCheck, Gauge, BookOpen, Save, History } from 'lucide-react'
...
export default function ProjectDetail() {
  ...
  const generateMeta = async () => {
    const meta = generateMetadata({
      ...
      audience: project.config.audience ?? 'adult',
      difficulty: project.config.difficulty ?? 'easy',
    })
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="card p-5 mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-fg-soft">Workflow Progress</span>
          <span className="text-sm text-fg-muted">{progress}% · Stage {currentIdx + 1} of {WORKFLOW_STAGES.length}</span>
        </div>
        <div className="h-2 bg-bg-soft rounded-full overflow-hidden mb-4">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition ${
                  current ? 'bg-brand-600 text-white' : done ? 'bg-success-50 text-success-600' : 'bg-bg-soft text-fg-muted hover:text-fg'
                }`}
            ...
            <div className="space-y-2">
              {versions.map((v) => (
                <div key={v.id} className="flex items-center justify-between text-sm">
                  <span className="text-fg-soft">{v.label ?? 'Autosave'}</span>
                  <span className="text-xs text-fg-muted">{new Date(v.created_at).toLocaleString()}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="card p-6 mb-6">
        ...
        {currentIdx < WORKFLOW_STAGES.length - 1 && (
          <div className="mt-6 pt-4 border-t border-border-soft flex justify-end">
            <button onClick={() => setStage(WORKFLOW_STAGES[currentIdx + 1].id)} className="btn-primary">
              Next: {WORKFLOW_STAGES[currentIdx + 1].label} <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        )}
      </div>
...
function StageContent({ stageId, project, exportConfig, coverConfig, complianceIssues, quality, onRunCompliance, onRunQuality, onGenerateMeta, onDownloadInterior, onDownloadCover }: any) {
  switch (stageId) {
    ...
    case 'niche-selection':
      return <NicheStage project={project} />
    case 'competition-analysis':
      return <CompetitionStage project={project} />
    case 'keyword-research':
      return <KeywordStage project={project} />
    case 'book-planning':
      return <PlanningStage project={project} />
    case 'interior-creation':
      return (
        <div className="space-y-3">
          <p className="text-sm text-fg-soft">Generate and preview your book interior. Use the Puzzle or Coloring Generator for full control.</p>
          <Link to="/puzzle-generator" className="btn-primary inline-flex">Open Puzzle Generator</Link>
          <div className="flex gap-2">
            <button onClick={onDownloadInterior} className="btn-outline"><Download className="w-4 h-4" /> Preview Interior PDF</button>
          </div>
        </div>
      )
    case 'cover-creation':
      return (
        <div className="space-y-3">
          <p className="text-sm text-fg-soft">Design your cover. The Cover Creator handles front, spine, and back with KDP-compliant dimensions.</p>
          <Link to="/cover-creator" className="btn-primary inline-flex">Open Cover Creator</Link>
          <button onClick={onDownloadCover} className="btn-outline"><Download className="w-4 h-4" /> Preview Cover PDF</button>
        </div>
      )
    case 'metadata-creation':
      return (
        <div className="space-y-3">
          <p className="text-sm text-fg-soft">Generate optimized metadata — title, description, keywords, and categories.</p>
    ...
    case 'quality-validation':
      return (
        <div className="space-y-4">
          <p className="text-sm text-fg-soft">Run automated quality checks across your book content.</p>
          ...
          {project.quality_score != null && (
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className={`text-3xl font-bold ${project.quality_score >= 80 ? 'text-success-600' : project.quality_score >= 60 ? 'text-warning-600' : 'text-danger-600'}`}>{project.quality_score}</div>
                <div className="text-sm text-fg-muted">/ 100 quality score</div>
              </div>
              <div className="space-y-1.5">
                {quality.checks.map((c: any) => (
                  <div key={c.field} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-fg-soft">{c.field}:</span>
    ...
    case 'compliance-validation':
      return (
        <div className="space-y-4">
          <p className="text-sm text-fg-soft">Validate your book against Amazon KDP requirements.</p>
          ...
          <div className="space-y-2">
            {complianceIssues.map((issue: any, i: number) => (
              <div key={i} className={`p-3 rounded-lg text-sm flex items-start gap-2 ${
                issue.level === 'error' ? 'bg-danger-50 text-danger-600' :
                issue.level === 'warning' ? 'bg-warning-50 text-warning-600' :
                ...
                <div>
                  ...
                  {issue.fix && <div className="text-xs mt-0.5 opacity-75">Fix: {issue.fix}</div>}
                </div>
              </div>
            ))}
          </div>
    ...
    case 'export':
      return (
        <div className="space-y-3">
          <p className="text-sm text-fg-soft">Export your complete KDP publishing package.</p>
          <div className="flex flex-wrap gap-2">
            <button onClick={onDownloadInterior} className="btn-primary"><Download className="w-4 h-4" /> Interior PDF</button>
            <button onClick={onDownloadCover} className="btn-outline"><Download className="w-4 h-4" /> Cover PDF</button>
          </div>
        </div>
      )
    case 'ready-for-kdp':
      return (
        <div className="text-center py-6">
          ...
          <p className="text-sm text-fg-muted mt-1 max-w-md mx-auto">
            Your book has passed all quality and compliance checks. Upload the exported PDFs to your Amazon KDP account to publish.
          </p>
        </div>
      )
    default:
...
function ResearchStage({ project }: { project: Project }) {
  return (
    <div className="space-y-3">
      <p className="text-sm text-fg-soft">Research the Amazon marketplace for opportunities. Visit the Market Research tool for detailed analysis.</p>
      <Link to="/market-research" className="btn-primary inline-flex">Open Market Research</Link>
      <div className="grid sm:grid-cols-2 gap-3 mt-2">
        {[
          { label: 'Bestselling books', desc: 'Top performers in your category' },
          { label: 'Trending books', desc: 'Rising titles gaining momentum' },
          { label: 'Seasonal trends', desc: 'Time-sensitive opportunities' },
          { label: 'Gap analysis', desc: 'Underserved areas in the market' },
        ].map((item) => (
          <div key={item.label} className="card p-4 bg-bg-soft">
            <div className="font-medium text-fg text-sm">{item.label}</div>
...
function NicheStage({ project }: { project: Project }) {
  return (
    <div className="space-y-3">
      <p className="text-sm text-fg-soft">Find profitable niches with manageable competition.</p>
      <Link to="/niche-finder" className="btn-primary inline-flex">Open Niche Finder</Link>
    </div>
  )
}

function CompetitionStage({ project }: { project: Project }) {
  return (
    <div className="space-y-3">
      <p className="text-sm text-fg-soft">Analyze competing books to understand positioning and pricing.</p>
      <Link to="/market-research" className="btn-primary inline-flex">Analyze Competition</Link>
    </div>
  )
}

function KeywordStage({ project }: { project: Project }) {
  return (
    <div className="space-y-3">
      <p className="text-sm text-fg-soft">Research primary, secondary, and long-tail keywords.</p>
      <Link to="/keyword-research" className="btn-primary inline-flex">Open Keyword Research</Link>
    </div>
  )
}

function PlanningStage({ project }: { project: Project }) {
  return (
    <div className="space-y-3">
      <p className="text-sm text-fg-soft">Plan your book structure: trim size, page count, difficulty, and format.</p>
      <div className="grid sm:grid-cols-3 gap-3">
        <div className="card p-3 bg-bg-soft">
          <div className="text-xs text-fg-muted">Trim Size</div>
          <div className="text-sm font-medium text-fg">{project.config.trimSize ?? '8.5x11'}</div>
        </div>
        <div className="card p-3 bg-bg-soft">
          <div className="text-xs text-fg-muted">Page Count</div>
          <div className="text-sm font-medium text-fg">{project.config.pageCount ?? 60}</div>
        </div>
        <div className="card p-3 bg-bg-soft">
          <div className="text-xs text-fg-muted">Difficulty</div>
          <div className="text-sm font-medium text-fg capitalize">{project.config.difficulty ?? 'easy'}</div>
        </div>
      </div>
    </div>
  )
