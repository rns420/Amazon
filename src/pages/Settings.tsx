...
import { Settings as SettingsIcon, Ruler, Globe, Bell, Palette, FileDown } from 'lucide-react'
      ...
      <div className="space-y-6">
        ...
        <Section icon={<Ruler className="w-5 h-5" />} title="Defaults" desc="Default trim size and units for new projects">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className="input" value={prefs.defaultTrimSize ?? '8.5x11'} onChange={(e) => save({ ...prefs, defaultTrimSize: e.target.value })}>
                {KDP_TRIM_SIZES.map((t) => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="label">Units</label>
              <select className="input" value={prefs.units ?? 'inches'} onChange={(e) => save({ ...prefs, units: e.target.value })}>
        ...
        <Section icon={<Globe className="w-5 h-5" />} title="Language & Region" desc="Interface language and marketplace">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className="input" value={prefs.language ?? 'en'} onChange={(e) => save({ ...prefs, language: e.target.value })}>
            ...
            <div>
              <label className="label">Default Marketplace</label>
              <select className="input" value={prefs.marketplace ?? 'US'} onChange={(e) => save({ ...prefs, marketplace: e.target.value })}>
        ...
        <Section icon={<FileDown className="w-5 h-5" />} title="Export Preferences" desc="Default export settings">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className="input" value={prefs.pdfQuality ?? 'high'} onChange={(e) => save({ ...prefs, pdfQuality: e.target.value })}>
            ...
            <div>
              <label className="label">Include Solutions</label>
              <select className="input" value={prefs.includeSolutions ?? 'yes'} onChange={(e) => save({ ...prefs, includeSolutions: e.target.value })}>
        ...
        <Section icon={<Bell className="w-5 h-5" />} title="Notifications" desc="Alert and notification preferences">
          <label className="flex items-center justify-between cursor-pointer">
            <span className="text-sm text-fg-soft">Toast notifications</span>
            <input type="checkbox" defaultChecked={prefs.toasts !== false} onChange={(e) => save({ ...prefs, toasts: e.target.checked })} className="w-5 h-5 rounded" />
          </label>
          <label className="flex items-center justify-between cursor-pointer">
            <span className="text-sm text-fg-soft">Activity logging</span>
            <input type="checkbox" defaultChecked={prefs.activityLog !== false} onChange={(e) => save({ ...prefs, activityLog: e.target.checked })} className="w-5 h-5 rounded" />
          </label>
        </Section>
      </div>
    </div>
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-lg bg-brand-50 text-brand-600 flex items-center justify-center">{icon}</div>
        <div>
