        ...
        <div className="lg:col-span-1">
          <div className="card p-5 space-y-4">
            <div>
              ...
              <input className="input" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g. Word Search, Mandala, Sudoku" />
            </div>
            <div>
              <label className="label">Book Type</label>
              <select className="input" value={bookType} onChange={(e) => setBookType(e.target.value as any)}>
                <option value="puzzle">Puzzle Book</option>
          ...
          {meta ? (
            <div className="card p-6 space-y-5">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-fg">Generated Metadata</h3>
                <button onClick={copyAll} className="btn-outline text-sm"><Copy className="w-4 h-4" /> Copy All</button>
              </div>
              <Field label="Title" value={meta.title ?? ''} />
              {meta.subtitle && <Field label="Subtitle" value={meta.subtitle} />}
              <div className="card p-3 bg-bg-soft text-sm text-fg">{meta.short_description ?? ''}</div>
              <div>
                <div className="label">Full Description</div>
                <div className="card p-4 bg-bg-soft whitespace-pre-wrap text-sm text-fg-soft leading-relaxed">{meta.description}</div>
              </div>
              <div>
                <div className="label">Backend Keywords ({meta.keywords?.length})</div>
                <div className="flex flex-wrap gap-2">
                  {meta.keywords?.map((k, i) => <span key={i} className="badge bg-bg-soft text-fg-soft">{k}</span>)}
                </div>
              </div>
              <div>
                <div className="label">Categories</div>
                <div className="space-y-1">
                  {meta.categories?.map((c, i) => <div key={i} className="text-sm text-fg-soft">{c}</div>)}
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <Field label="Language" value={meta.language ?? ''} />
                <Field label="Reading Age" value={meta.reading_age ?? ''} />
                <Field label="Audience" value={meta.audience ?? ''} />
              </div>
            </div>
          ) : (
            <div className="card p-10 text-center text-fg-muted text-sm">
              Configure your book details and click Generate to create optimized metadata.
            </div>
          )}
        </div>
      </div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="card p-3 bg-bg-soft text-sm text-fg">{value}</div>
    </div>
  )
}
