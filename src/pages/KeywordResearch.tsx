...
import { KeyRound, Search, TrendingUp } from 'lucide-react'
      ...
      <div className="card p-5 mb-6">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input className="input pl-9" value={topic} onChange={(e) => setTopic(e.target.value)} placeholder="e.g. word search, sudoku, coloring…" onKeyDown={(e) => e.key === 'Enter' && research()} />
          </div>
      ...
      {results && (
        <div className="card overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-bg-soft text-fg-muted text-xs uppercase">
              <tr>
                <th className="text-left px-4 py-3 font-medium">Keyword</th>
                <th className="text-left px-4 py-3 font-medium">Type</th>
                <th className="text-left px-4 py-3 font-medium">Search Intent</th>
                ...
                <tr key={i} className="hover:bg-bg-soft transition">
                  ...
                  <td className="px-4 py-3">
                    <span className={`badge ${
                      k.type === 'Primary' ? 'bg-brand-50 text-brand-700' :
                      k.type === 'Secondary' ? 'bg-accent-50 text-accent-600' :
                      k.type === 'Long-tail' ? 'bg-success-50 text-success-600' :
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-bg-soft rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${k.difficulty < 25 ? 'bg-success-500' : k.difficulty < 45 ? 'bg-warning-500' : 'bg-danger-500'}`} style={{ width: `${k.difficulty}%` }} />
                      </div>
                      <span className="text-xs text-fg-muted">{k.difficulty}</span>
                    </div>
                  </td>
                </tr>
              ))}
