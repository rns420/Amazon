...
import { Users, FolderKanban, Activity, CircleAlert as AlertCircle } from 'lucide-react'

export default function AdminPanel() {
  const { user } = useAuth()
  const [stats, setStats] = useState<{ projects: number; activity: number; users: number } | null>(null)
  ...
  useEffect(() => {
    (async () => {
      ...
      setStats({ projects: projects ?? 0, activity: activity ?? 0, users: 1 })
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard label="Users" value={stats!.users} icon={<Users className="w-5 h-5" />} accent="bg-success-50 text-success-600" />
