...
export default function ColoringGenerator() {
  ...
  // Generate a simple SVG line-art preview based on theme
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      <PageHeader
        ...
        subtitle="Create printable coloring book interiors with clean line art"
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="card p-5 space-y-4">
            ...
            <div>
              ...
              <input type="number" min={24} max={200} className="input" value={pageCount} onChange={(e) => setPageCount(Number(e.target.value))} />
            </div>
            <div>
              <label className="label">Trim Size</label>
              <select className="input" value={trimSize} onChange={(e) => setTrimSize(e.target.value)}>
                {KDP_TRIM_SIZES.filter((t) => t.bleed).map((t) => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
            </div>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={singleSided} onChange={(e) => setSingleSided(e.target.checked)} className="rounded" />
              <span className="text-sm text-fg-soft">Single-sided pages</span>
            </label>
          </div>
        ...
        <div className="lg:col-span-2">
          <div className="card p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-fg">Page Preview</h3>
            </div>
            <div className="bg-white rounded-lg border border-border p-8 flex items-center justify-center min-h-[400px]">
              <svg viewBox="0 0 300 300" className="w-full max-w-md" style={{ filter: 'drop-shadow(0 1px 2px rgb(0 0 0 / 0.05))' }}>
                {patterns.map((d, i) => (
                  <path key={i} d={d} fill="none" stroke="#1e293b" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                ))}
              </svg>
            </div>
            <p className="text-xs text-fg-muted mt-3 text-center">
              Preview is a sample line-art pattern. The exported PDF includes {pageCount} pages of {COLORING_THEMES.find((t) => t.id === theme)?.label.toLowerCase()} designs{singleSided ? ' (single-sided)' : ''}.
            </p>
          </div>
        </div>
      </div>
