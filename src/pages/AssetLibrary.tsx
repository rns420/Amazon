    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input className="input pl-9" placeholder="Search assets or tags…" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title="No assets yet"
          message="Assets you create or export will appear here. Generate a puzzle book or cover to populate your library."
        />
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {filtered.map((a) => (
            <div key={a.id} className="card p-4 hover:shadow-pop transition">
              <div className="w-full aspect-square bg-bg-soft rounded-lg flex items-center justify-center mb-3">
              ...
              {a.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {a.tags.slice(0, 3).map((t) => (
                    <span key={t} className="badge bg-bg-soft text-fg-muted text-[10px]"><Tag className="w-2.5 h-2.5" />{t}</span>
                  ))}
                </div>
              )}
            </div>
